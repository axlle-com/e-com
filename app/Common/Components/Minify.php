...
class Minify
{
    public function html($input)
    {
        ...
        $input = preg_replace_callback('#<([^\/\s<>!]+)(?:\s+([^<>]*?)\s*|\s*)(\/?)>#s', static function($matches) {
            return '<' . $matches[1] . preg_replace('#([^\s=]+)(\=([\'"]?)(.*?)\3)?(\s+|$)#s', ' $1$2', $matches[2]) . $matches[3] . '>';
        }, str_replace("\r", "", $input));
        # Minify inline CSS declaration(s)
        if(str_contains($input, ' style=')) {
            $input = preg_replace_callback('#<([^<]+?)\s+style=([\'"])(.*?)\2(?=[\/\s>])#s', static function($matches) {
                return '<' . $matches[1] . ' style=' . $matches[2] . self::css($matches[3]) . $matches[2];
            }, $input);
        }
        if(str_contains($input, '</style>')) {
            $input = preg_replace_callback('#<style(.*?)>(.*?)</style>#is', static function($matches) {
                return '<style' . $matches[1] . '>' . self::css($matches[2]) . '</style>';
            }, $input);
        }
        if(str_contains($input, '</script>')) {
            $input = preg_replace_callback('#<script(.*?)>(.*?)</script>#is', static function($matches) {
